...
- Passe die Klasse `Dice` aus Übungsaufgabe
  [ClassDiagrams02](../class-diagrams/class-diagrams02) anhand des abgebildeten
...
- Passe die Methode `boolean move(player: Player)` der Klasse `DiceGame` aus
  Übungsaufgabe [ClassDiagrams02](../class-diagrams/class-diagrams02) so an,
  dass jeder Spieler während des Spiels einmal die Möglichkeit hat, entweder nur
...
## Hinweis zur Klasse _HighValueDice_

Die Methode `void rollTheDice()` soll nur 4er, 5er und 6er "würfeln".

## Hinweis zur Klasse _LowValueDice_
...
<Exercise branchSuffix="polymorphism-04" />
