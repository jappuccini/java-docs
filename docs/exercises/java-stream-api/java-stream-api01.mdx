...
- Die Methode `List<Console> getAllConsolesSortedByLifespan()` soll alle
  Konsolen absteigend sortiert nach der Lebensspanne zurückgeben
...
- Die Methode `Map<Maker, List<Console>> getAllConsolesByMaker()` soll alle
  Konsolen gruppiert nach den Herstellern zurückgeben (_MICROSOFT:
...
<Exercise branchSuffix="stream-api-01" />
